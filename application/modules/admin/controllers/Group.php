...
header('Access-Control-Allow-Origin: *');
	...
	public function saveedit()
	{
        ...
        $this->form_validation->set_rules('groupID', 'Invalid Request', 'required');
        $this->form_validation->set_rules('langID', 'Group Language', 'required');
        $this->form_validation->set_rules('grouptypeID', 'Group Type', 'required');
        $this->form_validation->set_rules('name', 'Name', 'required');
        $this->form_validation->set_rules('description', 'Group Description', 'required');
        ...
        else
        {
        	...
        	$groupImage = 0;
        	if(isset($_FILES['groupImage'])  && !empty($_FILES['groupImage']['name']))
	        {
	        	...
	        	try {
						...
						$mediaID = $this->Group_model->addMedia($imgData);
						$this->Group_model->update($groupID , array("groupImage"=>$mediaID));
					}
        	...
        	if($this->Group_model->update($groupID , $arr))
        	{
        		// die('ok');

                $this->session->set_flashdata('success', 'Group update successfully!');
                redirect('admin/group/all');
        	}
        	else
        	{
        		$this->session->set_flashdata('error', 'server error');
	...
	public function create()
	{
		...
		$this->template->admin('addgroup',$data);
	}
	public function edit($id = 0)
	{
		...
		$data['types']= $this->Group_model->getGrouptype(array());
		$data['assets']= base_url('assets/admin/');
		$data['edit']= $this->Group_model->get(array('groupID'=> $id));
	...
	public function all()
	{
		$data= array();
		$data['assets']= base_url('assets/admin/');
		$data['data']= $this->Group_model->get(array('status'=> 1));
		
		$this->template->admin('allgroups',$data);
	}
	public function acceptStatus($groupID, $status)
	{
		$this->Group_model->update($groupID, array('status'=>$status));
		if($status == 1)
		{
			$this->session->set_flashdata('success', 'Group Accepted!');
		}
		else
			{
			$this->session->set_flashdata('success', 'Group Rejected!');
		}
		redirect($_SERVER['HTTP_REFERER']);
	}
	public function accept()
	{
		$data= array();
		$data['assets']= base_url('assets/admin/');
		$data['data']= $this->Group_model->get(array('status'=> 0));
		
		$this->template->admin('accept-group-request',$data);
	}
	public function delete($id = 0)
	{
		if($id)
		{
			if($this->Group_model->update($id, array('status'=> 3)))
        	{
