...
header('Access-Control-Allow-Origin: *');
	...
	public function saveedit()
	{
        ...
        { 
        	...
        	if($this->input->post('author_id') && $this->input->post('author_id') > 0)
        	{
        		$author_id = $this->input->post('author_id');
	...
	public function create()
	{
		...
		$this->template->admin('addbook',$data);
	}
	public function  edit($bookID)
	{
		$data= array();
		$data['edit']= $this->Book_model->get(array('bookID'=>$bookID));
	...
	public function all()
	{
		...
		if($user->roleID == 1)
		{
			$data['data'] =  $books = $this->Book_model->get(array('status'=> 0));
		}
		else
		{
			$data['data'] =  $books = $this->Book_model->get(array('status'=> 0,'uid'=>$user->UserID));
		}
		...
		$data['data'] = $obooks;
		$this->template->admin('allbooks',$data);
	}
	public function delete($id = 0)
	{
		if($id)
		{
			if($this->Book_model->update($id, array('status'=> 1)))
        	{
                $this->session->set_flashdata('success', 'Link delete successfully!');
        	}
        	else
        	{
        		$this->session->set_flashdata('error', 'server error');
